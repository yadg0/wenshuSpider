...
public class HttpClientDownloader extends AbstractDownloader {
	...
	private String redirectErrPage="waf_verify.htm";
	...
	public HttpResponse download(HttpRequest request, int timeout) throws DownloadException {
		...
		if(request instanceof HttpPostRequest) {//post
			...
			if(log.isDebugEnabled()) {
				log.debug("post fields..." + JSON.toJSONString(fields));
			}
			try {
		...
		if(SpiderThreadLocal.get()!=null)isMobile= SpiderThreadLocal.get().getEngine().isMobile();
		...
		if(proxys != null && isProxy) {
			...
			if(proxy != null) {
				log.info("proxy:" + proxy.getHostName()+":"+proxy.getPort());
				configBuilder.setProxy(proxy);
				configBuilder.setConnectTimeout(timeout);//如果走代理，连接超时时间固定为1s
			}
		}
		reqObj.setConfig(configBuilder.build());
		...
		try {
			...
			cookieContext.getCookieStore().getCookies();
//			response.getHeaders("Set-Cookie");
			otherProcess(reqObj,resp);
			resp.setStatus(status);
			if(status == 302 || status == 301) {
				String redirectUrl = response.getFirstHeader("Location").getValue();
				log.info("重定向了。。。"+redirectUrl);
				if(redirectUrl.indexOf(redirectErrPage)>-1){//重定向到了错误页面
					if(proxy != null) {
						proxys.failure(proxy.getHostName(), proxy.getPort());
					}
					throw new DownloadServerException("重定向到了错误页面");
				}else{
				...
				if(!isImage(contentType)) { 
					...
					if(!isValidContent(content)){
						throw new DownloadServerException("内容获取失败！");
					}
					resp.setContent(content);
				}
//				log.info("下载成功。。。proxy:=="+proxy);
			} else{
//				log.info("下载失败。。。proxy:=="+proxy);
				//404，500等
				if(proxy != null) {
					proxys.failure(proxy.getHostName(), proxy.getPort());
				}
	...
	private List<String> validContentCharact=new ArrayList<>();
	public void setValidContentCharact(String...strings){
		for(String s:strings){
			validContentCharact.add(s);
		}
	}
	private boolean isValidContent(String content) {
		if(content.indexOf("HtmlNotExist")>-1){
			return false;
		}
		if(content.indexOf("请开启JavaScript并刷新该页")>-1){
			return false;
		}
		return true;
	}
